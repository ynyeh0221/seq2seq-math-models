...
def generate_subtraction_data(num_samples=20000, max_val=99):
    data = []
    for _ in range(num_samples):
        a, b = random.randint(1, max_val), random.randint(1, max_val)
        if a < b:
            a, b = b, a  # ensure result is non-negative
        res = a - b
        src = f"{a}-{b}="
        trg = str(res)
        data.append((src, trg))
...
train_data = generate_subtraction_data(20000)
...
# Save model
torch.save(model.state_dict(), '../subtraction_rnn_reverse_model.pth')
...
def evaluate(model, loader, max_length=10):
    ...
    with torch.no_grad():
        for src, trg in loader:
            ...
            print(f"Input: {input_str[::-1]} Predicted: {pred_str} True: {true_str}")
