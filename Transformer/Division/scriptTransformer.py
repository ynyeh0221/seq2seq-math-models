...
# TODO Modify the script to handle cases with zero
...
# Function to generate division data
def generate_division_data(num_samples=10000, max_val=99):
    data = []
    for _ in range(num_samples):
        b = random.randint(1, max_val)
        res = random.randint(1, max_val)
        a = b * res  # Ensures integer division
        src = f"{a}/{b}="
        trg = str(res)
        data.append((src, trg))
    return data
...
train_data = generate_division_data(10000)
test_data = generate_division_data(500)
...
torch.save(model.state_dict(), '../division_model.pth')
