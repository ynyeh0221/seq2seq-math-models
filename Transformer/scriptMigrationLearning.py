...
# Combined tokenizer (supports +, -, x, /)
class Tokenizer:
    def __init__(self):
        self.vocab = [PAD, SOS, EOS, '+', '-', 'x', '/', '='] + [str(i) for i in range(10)]
...
# Combined arithmetic data generation
def generate_combined_data(num_samples=10000, max_val=99):
    data = []
...
train_data = generate_combined_data(20000)
test_data = generate_combined_data(1000)
...
class TransformerModel(nn.Module):
    def __init__(self, vocab_size, embed_dim, num_heads, num_layers=4):
        super().__init__()
...
# Migration learning: loading pretrained parameters from four models
model_paths = ["addition_model.pth", "subtraction_model.pth", "multiplication_model.pth", "division_model.pth"]
avg_state_dict = None

for path in model_paths:
    pretrained_dict = torch.load(path)
    if avg_state_dict is None:
        avg_state_dict = pretrained_dict
    else:
        for key in avg_state_dict:
            avg_state_dict[key] += pretrained_dict[key]

for key in avg_state_dict:
    avg_state_dict[key] = avg_state_dict[key] / len(model_paths)

model_dict = model.state_dict()
pretrained_dict = {k: v for k, v in avg_state_dict.items() if k in model_dict and v.size() == model_dict[k].size()}
